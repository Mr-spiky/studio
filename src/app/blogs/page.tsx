        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
          <main className="md:col-span-3">
            <div className="grid gap-12 lg:grid-cols-2">
              {blogPosts.map((post) => (
                <Card key={post.id} className="flex flex-col overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1 group">
                  ...
                  <CardHeader>
                    ...
                    <CardTitle className="font-headline text-2xl leading-snug">{post.title}</CardTitle>
                  </CardHeader>
                  <CardContent className="flex-grow">
                    <p className="text-muted-foreground line-clamp-3">{post.description}</p>
                  </CardContent>
                  <div className="border-t p-6">
                      <Link href={`/blogs/${post.id}`} className="font-semibold text-primary transition-colors hover:text-accent group-hover:text-accent">
          ...
          <aside className="md:col-span-1">
            <div className="sticky top-24 space-y-10">
              <div>
                <h3 className="text-xl font-bold text-primary mb-4 border-b pb-2">Categories</h3>
              ...
              <div>
                <h3 className="text-xl font-bold text-primary mb-4 border-b pb-2">Recent Posts</h3>
                <div className="space-y-4">
                  {blogPosts.slice(0, 2).map(post => (
                    <Link key={post.id} href={`/blogs/${post.id}`} className="flex items-start gap-4 group">
                      ...
                      <div>
                        <p className="font-semibold text-primary group-hover:text-accent transition-colors line-clamp-2">{post.title}</p>
